...
def display_closest_edge(vertices1 , vertices2 , v1v2_closest_v2idxs , distance_color , save_path , sampling = 10):
    new_vtx  = []
    new_face = []
    new_rgb  = []
    j = 0
    ...
    for i , v1 in enumerate(vertices1):
        if i%sampling != 0:
            continue
        new_vtx.append(v1)
        new_vtx.append(vertices2[v1v2_closest_v2idxs[i]])
        new_vtx.append(vertices2[v1v2_closest_v2idxs[i]])
...
def point_to_point_distance(vertices1 , vertices2):
 
    L2norm = torch.cdist(vertices1 , vertices2 , p=2.0)
    v1v2_closest_v2idxs = L2norm.argmin(1, keepdim=True)

    v1v2_closest_L2norms = L2norm.gather(1, v1v2_closest_v2idxs).squeeze()
    v1v2_closest_v2idxs    = v1v2_closest_v2idxs.squeeze() 

    v2v1_closest_v1idxs = L2norm.argmin(0, keepdim=True)
    v2v1_closest_L2norm = L2norm.gather(0, v2v1_closest_v1idxs).squeeze()
    v2v1_closest_v1idxs = v2v1_closest_v1idxs.squeeze()

    return v1v2_closest_L2norms , v1v2_closest_v2idxs , v2v1_closest_L2norm , v2v1_closest_v1idxs
...
def compute_normal_consistency(normal_similarity ,vertices_normal_GT , normal_pred_tmp, normal_hadamard , normal_dot , normal_norm1, normal_norm2):
    normal_hadamard   = vertices_normal_GT * normal_pred_tmp
    normal_dot        = np.sum(normal_hadamard , axis = 1)    
    normal_norm1      = np.linalg.norm(vertices_normal_GT , axis=1)
    normal_norm2      = np.linalg.norm(normal_pred_tmp , axis=1)
    normal_similarity = normal_dot / (normal_norm1 * normal_norm2)
    return normal_similarity

def compute_normal_consistency_point2point(normal_similarity , vertices_normal_GT , vertices_normal_pred  , index_np , normal_hadamard , normal_dot , normal_norm1 , normal_norm2):
    #normal_pred_tmp = np.zeros((,3) , np.float32)
    normal_pred_tmp = vertices_normal_pred[index_np]

    normal_similarity = compute_normal_consistency(normal_similarity ,vertices_normal_GT , normal_pred_tmp, normal_hadamard , normal_dot , normal_norm1, normal_norm2)
    return normal_similarity
...
def load_factory(path , device):
    ...
    return verts , verts_tensor , face , vertices_normal

def chamfer(GT_path, pred_path , GT_to_Pred_save_path , Pred_to_GT_save_path ,device = None):
    verts_GT  , verts_GT_tensor   , face_GT   , vertices_normal_GT    = load_factory(GT_path   , device)
    verts_pred, verts_pred_tensor , face_pred , vertices_normal_pred  = load_factory(pred_path , device)

    print("start computing chamfer distance")
    distance_gt2pred , index_gt2pred , distance_pred2gt , index_pred2gt = point_to_point_distance(verts_GT_tensor[0] , verts_pred_tensor[0])
    print("end computing chamfer distance")

    ###debug color ###
    distance_color_gt2pred = save_color_mesh(distance_gt2pred , verts_GT , face_GT , GT_to_Pred_save_path + "_dist.ply" , vmin=0.0 , vmax=1e-1)
    distance_color_pred2gt = save_color_mesh(distance_pred2gt , verts_pred , face_pred , Pred_to_GT_save_path + "_dist.ply" , vmin=0.0 , vmax=1e-1)

    display_closest_edge(verts_GT , verts_pred , index_gt2pred , distance_color_gt2pred , GT_to_Pred_save_path + "_closest_edge.ply" )
    display_closest_edge(verts_pred , verts_GT , index_pred2gt , distance_color_pred2gt , Pred_to_GT_save_path + "_closest_edge.ply" )

    #######################################################
    ### GTMesh's point 2 PredinctedMesh's point distance
    #######################################################
    #indexはdist_typeに依らず、faceのindexを返す
    index_np     = index_gt2pred.to('cpu').detach().numpy().copy()   

    loss_chamfer_gt2pred = torch.nanmean(distance_gt2pred)
    
    
    #compute normal
    normal_hadamard   = np.zeros((len(distance_gt2pred) , 3) , np.float32)
    normal_similarity = np.zeros( len(distance_gt2pred)      , np.float32)
    normal_dot        = np.zeros( len(distance_gt2pred)      , np.float32)
    normal_norm1      = np.zeros( len(distance_gt2pred)      , np.float32)
    normal_norm2      = np.zeros( len(distance_gt2pred)      , np.float32)
   
    normal_similarity = compute_normal_consistency_point2point(normal_similarity , vertices_normal_GT , vertices_normal_pred , index_np , normal_hadamard , normal_dot , normal_norm1 , normal_norm2)
    ...
    #######################################################
    ### PredinctedMesh's point 2 GTMesh's point distance ###
    #######################################################
    #indexはdist_typeに依らず、faceのindexを返す
    index_np     = index_pred2gt.to('cpu').detach().numpy().copy()   

    loss_chamfer_pred2gt = torch.nanmean(distance_pred2gt)
    
    #compute normal
    normal_hadamard   = np.zeros((len(distance_pred2gt) , 3) , np.float32)
    normal_similarity = np.zeros( len(distance_pred2gt)      , np.float32)
    normal_dot        = np.zeros( len(distance_pred2gt)      , np.float32)
    normal_norm1      = np.zeros( len(distance_pred2gt)      , np.float32)
    normal_norm2      = np.zeros( len(distance_pred2gt)      , np.float32)
   
    normal_similarity = compute_normal_consistency_point2point(normal_similarity , vertices_normal_pred , vertices_normal_GT , index_np , normal_hadamard , normal_dot , normal_norm1 , normal_norm2)
    ...
    summary_distance_gt2pred = distance_gt2pred.to('cpu').detach().numpy().copy() * ((1 - normal_similarity_gt2pred ** 2) ** (1/2))
    summary_distance_pred2gt = distance_pred2gt.to('cpu').detach().numpy().copy() * ((1 - normal_similarity_pred2gt ** 2) ** (1/2))
    ...
    return (loss_chamfer_gt2pred * 1000, loss_normal_gt2pred , distance_gt2pred , normal_similarity_gt2pred \
          , loss_chamfer_pred2gt * 1000, loss_normal_pred2gt , distance_pred2gt , normal_similarity_pred2gt \
...
def main():
    ...
    p2p_chamfer_save_path = os.path.join(args.pred_dir_path , "p2p")
    if_not_exists_makedir(p2p_chamfer_save_path , confirm_delete = False)
    
    result_csv = os.path.join(p2p_chamfer_save_path,"chamfer_result.csv")  #save_path
    
    gt2pred_hist_dir_path = os.path.join(p2p_chamfer_save_path , "gt2pred_hist")
    pred2gt_hist_dir_path = os.path.join(p2p_chamfer_save_path , "pred2gt_hist")
    ...
    gt2pred_summary_hist_dir_path = os.path.join(p2p_chamfer_save_path , "gt2pred_summary_hist")
    pred2gt_summary_hist_dir_path = os.path.join(p2p_chamfer_save_path , "pred2gt_summary_hist")
    ...
    GT_to_Pred_RGB_save_dir_path = os.path.join(p2p_chamfer_save_path , "GT_to_Pred_RGB")
    Pred_to_GT_RGB_save_dir_path = os.path.join(p2p_chamfer_save_path , "Pred_to_GT_RGB")
    ...
    for (gt_path , pred_path ,id) in tqdm.tqdm(zip(gt_paths,pred_paths,ids)):
        ...
        if os.path.isfile(pred_path) == False or os.path.isfile(gt_path) == False:
            print("no that file!!")
            continue
        GT_to_Pred_summary_RGB_save_path = os.path.join(GT_to_Pred_RGB_save_dir_path , "GT_to_Pred_RGB_"+ id)
        Pred_to_GT_summary_RGB_save_path = os.path.join(Pred_to_GT_RGB_save_dir_path , "Pred_to_GT_RGB_"+ id)
        ...
        loss_chamfer_summary , loss_summary_gt2pred , loss_summary_pred2gt                                  = chamfer(gt_path, pred_path , GT_to_Pred_summary_RGB_save_path , Pred_to_GT_summary_RGB_save_path , device)
...
if __name__ == "__main__":
    ...
    v2 = torch.Tensor([[4,4,4],
                       [5,5,5],
                       [3,3,3]]).cuda()
    v1v2_closest_L2norms , v1v2_closest_v2idxs , v2v1_closest_L2norm , v2v1_closest_v1idxs = point_to_point_distance(v1 , v2)
    """
