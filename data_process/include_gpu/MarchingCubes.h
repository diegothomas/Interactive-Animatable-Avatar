...
int Indexing(float ***sdf, int ***IndexVal, int ***Offset, my_int3 size_grid, float iso) {
    ...
    int faces_counter = 0;
    for (int x = 0; x < size_grid.x-1; x++) {
        for (int y = 0; y < size_grid.y-1; y++) {
            for (int z = 0; z < size_grid.z-1; z++) {
...
void InnerLoop_MC(float *vertices, float *normals, int *faces, int x, float ***sdf, int ***IndexVal, int ***Offset, my_int3 size_grid, my_float3 center_grid, float res, float iso) {
    
    for (int y = 1; y < size_grid.y-2; y++) {
        if (x == 1)
            cout << 100.0f*float(y)/float(size_grid.y) << "%\r";
 ...
 */
int *MarchingCubes(float ***sdf, float **vertices, float **normals, int **faces, my_int3 size_grid, my_float3 center_grid, float res, float iso) {
    ...
    *faces = new int [3*nb_faces];
    ...
    for (int x = 1; x < size_grid.x-2; x++) {
        ...
        my_threads.push_back( std::thread(InnerLoop_MC, *vertices, *normals, *faces, x, sdf, IndexVal, Offset, size_grid, center_grid, res, iso) );
    }
