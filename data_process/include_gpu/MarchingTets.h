...
int *MarchingTetrahedra(float *TSDF, float *volume_weights, float *Vertices, float *skin_weights, int *Faces, float *Nodes, int *Tetra, int *Edges_row_ptr, int *Edges_columns, int nb_nodes, int nb_tets, int nb_edges, float m_iso = 0.0) {
    ...
    // 3. Remove null vertices
    int* indices = new int[nb_edges];
    int count_v = 0;
    for (int idx = 0; idx < nb_edges; idx++) {
        if (Vertices[3 * idx] != 0.0f || Vertices[3 * idx + 1] != 0.0f || Vertices[3 * idx + 2] != 0.0f) {
            Vertices[3 * count_v] = Vertices[3 * idx];
            Vertices[3 * count_v + 1] = Vertices[3 * idx + 1];
            Vertices[3 * count_v + 2] = Vertices[3 * idx + 2];
            Vertices[3 * idx] = 0.0f;
            Vertices[3 * idx + 1] = 0.0f;
            Vertices[3 * idx + 2] = 0.0f;

            for (int s = 0; s < 24; s++) {
                skin_weights[24 * count_v + s] = skin_weights[24 * idx + s];
            }
    ...
    // 3. Remove null faces & Reorganize faces
    int count_f = 0;
    for (int idx = 0; idx < 2 * nb_tets; idx++) {
        if ((Faces[3 * idx] != 0 || Faces[3 * idx + 1] != 0 || Faces[3 * idx + 2] != 0) &&
            (indices[Faces[3 * idx]] != -1 && indices[Faces[3 * idx+1]] != -1 && indices[Faces[3 * idx+2]] != -1)) {
            Faces[3 * count_f] = indices[Faces[3 * idx]];
            Faces[3 * count_f + 1] = indices[Faces[3 * idx + 1]];
            Faces[3 * count_f + 2] = indices[Faces[3 * idx + 2]];
            Faces[3 * idx] = 0;
            Faces[3 * idx + 1] = 0;
            Faces[3 * idx + 2] = 0;
            count_f++;
