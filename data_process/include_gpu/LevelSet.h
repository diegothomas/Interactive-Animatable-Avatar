...
void InnerLoop(float ***volume, int i, float *vertices, int *faces, float *normals, int nb_faces, my_int3 size_grid, my_float3 center_grid, float res, float disp) {
    ...
    float bbox_x = 0.1f; // 0.57f;
    float bbox_y = 0.08f; // 0.57f;
    float shift_y = 0.25f;
    float alpha = 0.5f;
    ...
    for (int j = 0; j < size_grid.y; j++) {
        ...
        for (int k = 0; k < size_grid.z; k++) {
            ...
            float curr_disp = 0.05f;
            /*if (fabs(p0.x) > bbox_x) {
                curr_disp = min(0.05f, alpha * (fabs(p0.x) - bbox_x));
            }
            if (fabs(p0.y - shift_y) > bbox_y) {
                if (p0.y - shift_y > 0.0f)
                    curr_disp = min(0.02f, curr_disp + alpha * (fabs(p0.y - shift_y) - bbox_y));
                else
                    curr_disp = min(0.05f, curr_disp + alpha * (fabs(p0.y - shift_y) - bbox_y));
            }*/
            ...
            volume[i][j][k] = int(intersections) % 2 == 0 ? sdf- curr_disp : -sdf- curr_disp;
        }
    }
}
