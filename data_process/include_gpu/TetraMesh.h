...
void SkinWeightsFromSmplNearVoxelInnerLoop(float* volume_weights, bool* sw_valid, int i, float* vertices, float* skin_weights, int* faces, float* normals, int nb_nodes, int nb_faces, float* nodes, float thre_dist) {
    ...
    for (int idx = i; idx < i + nb_nodes / 100; idx++) {
        ...
        for (int f = 0; f < nb_faces; f++) {
            ...
            // Compute point to face distance
            float curr_dist = DistancePointFace3D(p0, p1, p2, p3, n, false);
            if (curr_dist < min_dist && curr_dist < thre_dist) {
                min_dist = curr_dist;
                // Compute skin weights
                SkinWeightsFromFace3D(s_w, &skin_weights[24 * faces[3 * f]], &skin_weights[24 * faces[3 * f + 1]], &skin_weights[24 * faces[3 * f + 2]], p0, p1, p2, p3, n);
                sw_valid[idx] = true;
        ...
        for (int k = 0; k < 24; k++) {
            if (sw_valid[idx] == true) {
                volume_weights[24 * idx + k] = s_w[k];
            }
            else {
                volume_weights[24 * idx + k] = 0.0f;
            }
        }
        delete[] s_w;
...
bool num_cmp_ascend(const struct Data& p, const struct Data& q) {
    return p.num < q.num;
}

void SkinWeightsfromJointsDistInnerLoop(float* volume_weights, int i, float* skeleton, int nb_nodes, float* nodes, int* KinTree_Table) {
    for (int idx = i; idx < i + nb_nodes / 100; idx++) {
        if (idx >= nb_nodes)
            break;
        //if (i == 0)
        ...
        my_float3 p = make_my_float3(nodes[3 * idx], nodes[3 * idx + 1], nodes[3 * idx + 2]);

        // Compute the smallest distance to the joints
        vector<Data> v;

        struct Data temp;
        for (int f = 0; f < 24; f++) {
            my_float3 skel_p = make_my_float3(skeleton[3 * f], skeleton[3 * f + 1], skeleton[3 * f + 2]);
            // Compute point to joint distance
            temp.id = f;
            temp.num = sqrt(dot(p - skel_p, p - skel_p));
            v.push_back(temp);
        }

        vector<Data> sort_v = v;

        //sort by distance
        stable_sort(sort_v.begin(), sort_v.end(), num_cmp_ascend);
        float* s_w = new float[24];
        for (int k = 0; k < 24; k++) {
            s_w[k] = 0.0f;
        }
        ...
        cout << endl << endl << endl;
        for (int k = 0; k < 24; k++) {
            cout << "idx : " << idx << "id : " <<  sort_v[k].id << " : " << sort_v[k].num <<endl;
        }*/

        //method 1 : Get from Top closest n joints
        /*
        int used_sw = 2;
        float sum = 0;
        for (int k = 0; k < used_sw; k++) {             //use top n sw
            s_w[sort_v[k].id] = exp(-40 * pow(sort_v[k].num,2));
            sum += s_w[sort_v[k].id];
        }

        for (int k = 0; k < used_sw; k++) {             //normalize
            s_w[sort_v[k].id] = s_w[sort_v[k].id] / sum;
        }
        for (int k = used_sw; k < 24; k++) {
            s_w[sort_v[k].id] = 0.0f;
        }
        */

        //method 2 : Get from Closest joints and that parents and children
        int closest_id = sort_v[0].id;

        //Get parent id
        int parent_id = KinTree_Table[2 * closest_id];

        //Get parent_parent id
        int parent_parent_id = -1;
        if (parent_id != -1) {
            parent_parent_id = KinTree_Table[2 * parent_id];
        }

        //Get parent_child id
        vector<int> parent_child_ids;
        for (int k = 0; k < 24; k++) {
            if (parent_id == KinTree_Table[2 * k]) {
                parent_child_ids.push_back(k);
            }
        }

        //Get children id and child_child id
        vector<int> child_ids;
        vector<int> child_child_ids;
        for (int k = 0; k < 24; k++) {
            if (closest_id == KinTree_Table[2 * k]) {       //child
                child_ids.push_back(k);
                for (int l = 0; l < 24; l++) {              //child_child
                    if (k == KinTree_Table[2 * l]) {
                        child_child_ids.push_back(l);
                    }
                }
        ...
        float sum = 0;
        int t = 60;
        s_w[closest_id] = exp(-t * pow(v[closest_id].num, 2));
        //sum += s_w[closest_id];
        // 
        //parent
        if (parent_id != -1) {
            s_w[parent_id] = exp(-t * pow(v[parent_id].num, 2));
            //sum += s_w[parent_id];
        }
        //parent_parent
        if (parent_parent_id != -1) {
            s_w[parent_parent_id] = exp(-t * 0.8 * pow(v[parent_parent_id].num, 2));
            //sum += s_w[parent_parent_id];
        }
        //parent_child
        for (int k = 0; k < size(parent_child_ids); k++) {
            if (s_w[parent_child_ids[k]] == 0.0f) {
                s_w[parent_child_ids[k]] = exp(-t * 0.6 * pow(v[parent_child_ids[k]].num, 2));
                //sum += s_w[parent_child_ids[k]];
            }
        }
        //child
        for (int k = 0; k < size(child_ids); k++) {
            if (s_w[child_ids[k]] == 0.0f) {
                s_w[child_ids[k]] = exp(-t * 0.6 * pow(v[child_ids[k]].num, 2));
                //sum += s_w[child_ids[k]];
            }
        }
        //child_child
        for (int k = 0; k < size(child_child_ids); k++) {
            if (s_w[child_child_ids[k]] == 0.0f) {
                s_w[child_child_ids[k]] = exp(-t * 0.4 * pow(v[child_child_ids[k]].num, 2));
                //sum += s_w[child_child_ids[k]];
            }
        }

        /*
        for (int k = 0; k < 24; k++) {             //normalize
            if (s_w[k] != 0.0f) {
                s_w[k] = s_w[k] / sum;
            }
            //cout << "s_w" << k << " : " << s_w[k] << endl;    //debug
        }
        */


        //debug parent&children ids
        /*
        cout << "closest_id : " << closest_id << endl;
        cout << "parent_id : " << parent_id << endl;
        cout << "size(child_ids) : " << size(child_ids) << endl;
        for (int k = 0; k < size(child_ids); k++) {
            cout << "child_id" << k << " : " << child_ids[k] << endl;
        }
        cout << "end" << endl << endl;
        */
...
void SumNeighborTetraInnerLoop(float* volume_weights, int* len_SkinWeights, float* in_weights, int i, int nb_nodes, float* nodes, int nb_tetra, int* tetras) {
    //cout << "nb_tetra : " << nb_tetra << " i + nb_nodes / 100 : " << i + nb_nodes / 100 << endl;
    ...
    for (int idx = i; idx < i + nb_nodes / 100; idx++) {
        ...
        //find neighbor node in the belonging boxel
        vector<int> nodes_tmp;
        for (int idt = 0; idt < nb_tetra; idt++) {
            //if (idx % 100000 == 0) cout << idt << endl;
            for (int j = 0; j < 4; j++) {
                if (idx == tetras[4 * idt + j]) {
                    for (int k = 0; k < 4; k++) {
                        if (idx != tetras[4 * idt + k]) {
                            //cout << "tetras :" << idx << " " << tetras[4 * idt + k] << endl;
                            nodes_tmp.push_back(tetras[4 * idt + k]);
                        }
                    }
                }
            }
        ...
        nodes_tmp.erase(std::unique(nodes_tmp.begin(), nodes_tmp.end()), nodes_tmp.end());
        for (int j = 0; j < 24; j++) volume_weights[24 * idx + j] = 0.0f;   //initialize
        len_SkinWeights[idx] = nodes_tmp.size();
        //sum the surrounding skinning weight
        for (int n = 0; n < len_SkinWeights[idx]; n++) {
            for (int j = 0; j < 24; j++) {
                volume_weights[24 * idx + j] += in_weights[24 * nodes_tmp[n] + j];
            }
        }

        //avg the surrounding skinning weight
        for (int j = 0; j < 24; j++) {
            if (volume_weights[24 * idx + j] != 0.0f) {
                volume_weights[24 * idx + j] = volume_weights[24 * idx + j] / len_SkinWeights[idx];
            }
            else {
                volume_weights[24 * idx + j] = 0.0f;
            }
        }
...
void SumNeighborTetraValidInnerLoop(float* volume_weights, bool* sw_valid_out, int* len_SkinWeights, float* in_weights, int i, int nb_nodes, float* nodes, int nb_tetra, int* tetras, bool* sw_valid) {
    //cout << "nb_tetra : " << nb_tetra << " i + nb_nodes / 100 : " << i + nb_nodes / 100 << endl;
    ...
    for (int idx = i; idx < i + nb_nodes / 100; idx++) {
        ...
        if (sw_valid[idx] == true) {
            for (int j = 0; j < 24; j++) {
                volume_weights[24 * idx + j] = in_weights[24 * idx + j];
                sw_valid_out[idx] = true;
            }
            continue;
        }
        //if (i == 0)
        ...
        for (int idt = 0; idt < nb_tetra; idt++) {
            ...
            for (int j = 0; j < 4; j++) {
                if (idx == tetras[4 * idt + j]) {
                    for (int k = 0; k < 4; k++) {
                        if (idx != tetras[4 * idt + k] && sw_valid[tetras[4 * idt + k]] == true) {
                            //cout << "tetras :" << idx << " " << tetras[4 * idt + k] << endl;
                            nodes_tmp.push_back(tetras[4 * idt + k]);
                        }
                    }
        ...
        for (int j = 0; j < 24; j++) volume_weights[24 * idx + j] = 0.0f;   //initialize
        len_SkinWeights[idx] = nodes_tmp.size();
        if (len_SkinWeights[idx] == 0) {
            sw_valid_out[idx] = false;
            continue;
        }
        sw_valid_out[idx] = true;
        //sum the surrounding valid skinning weight
        for (int n = 0; n < len_SkinWeights[idx]; n++) {
            for (int j = 0; j < 24; j++) {
                volume_weights[24 * idx + j] += in_weights[24 * nodes_tmp[n] + j];
            }
        ...
        //avg the surrounding valid skinning weight
        for (int j = 0; j < 24; j++) {
            if (volume_weights[24 * idx + j] != 0.0f) {
                volume_weights[24 * idx + j] = volume_weights[24 * idx + j] / len_SkinWeights[idx];
            }
...
public:
    ...
    TetraMesh(string path, float res = 1.0f): _res(res) {
        int *res_tetra = LoadPLY_Tet(path+string("TetraShell.ply"), &_Nodes, &_Faces, &_Tetra);
        _Nb_Nodes = res_tetra[0];
        _Nb_Faces = res_tetra[1];
        _Nb_tetra = res_tetra[2];
        _res = float(res_tetra[3]/1000.0f);
        cout << "ok" << endl;
        ...
        LoadWeights(path);
        //ComputeWeights(path);
        //ComputeWeightsFromSkel(path);     //no use
        //ComputeWeightsFromSmplNearVoxel(path);    //maybe best
        
        _isSurface = new bool[_Nb_Nodes];
    ...
    inline void SetNodes(float *in_nodes) {
        memcpy(_Nodes, in_nodes, 3 * _Nb_Nodes * sizeof(float));
    }

    inline void SetNormals(float* in_normals) {
        memcpy(_Normals, in_normals, 3 * _Nb_Nodes * sizeof(float));
    }
    ...
    int* GetSurface(float** vertices, float** normals, float** weights, int** faces) {
        *vertices = new float[3 * _Nb_Surface];
        *normals = new float[3 * _Nb_Surface];
        *weights = new float[24 * _Nb_Surface];
        int* index = new int[_Nb_Nodes];
        float* Vbuff = *vertices;
        float* Nbuff = *normals;
        float* Wbuff = *weights;
        ...
        for (int s = 0; s < _Nb_Surface; s++) {
            ...
            Nbuff[3 * s + 2] = _Normals[3 * _Surface[s] + 2];
            for (int k = 0; k < 24; k++)
                Wbuff[24 * s + k] = _SkinWeights[24 * _Surface[s] + k];
            index[_Surface[s]] = s;
        }
        *faces = new int[3 * _Nb_Faces];
        int* Fbuff = *faces;
    ...
    void SetSurface(float* vertices, float* normals) {
        for (int s = 0; s < _Nb_Surface; s++) {
            _Nodes[3 * _Surface[s]] = vertices[3*s];
            _Nodes[3 * _Surface[s] + 1] = vertices[3 * s + 1];
            _Nodes[3 * _Surface[s] + 2] = vertices[3 * s + 2];
            _Normals[3 * _Surface[s]] = normals[3 * s];
            _Normals[3 * _Surface[s] + 1] = normals[3 * s + 1];
            _Normals[3 * _Surface[s] + 2] = normals[3 * s + 2];
        }
        return;
    };

    void Save_Surface_weight(string save_path) {
        int* index = new int[_Nb_Nodes];
        float* Wbuff = new float[24 * _Nb_Surface];
        for (int s = 0; s < _Nb_Nodes; s++) {
            index[s] = -1;
        }
        ...
        //save surface skinning weight
        cout << "save_path : " << save_path + string("weights_surface.bin") << endl;
        auto file = std::fstream(save_path + string("weights_surface.bin"), std::ios::out | std::ios::binary);
        int size = 24 * _Nb_Surface;
        file.write((char*)Wbuff, size * sizeof(float));
        file.close();

        return;
    };
    
    inline float *LevelSet(float *vertices, int *faces, float *normals_face, int nb_vertices, int nb_faces, float disp) {
        ...
        return TetLevelSet_gpu(vertices, faces, normals_face, _Nodes, nb_vertices, nb_faces, _Nb_Nodes, disp);
    }
    
    inline int *MT(float *TSDF, float *volume_weights, float *Vertices, float *skin_weights, int *Faces, float m_iso = 0.0) {
        return MarchingTetrahedra(TSDF, _SkinWeights, Vertices, skin_weights, Faces, _Nodes, _Tetra, _Edges_row_ptr, _Edges_columns, _Nb_Nodes, _Nb_tetra, _Nb_Edges, m_iso);
        //return MarchingTetrahedraNaive(TSDF, Vertices, Faces, _Nodes, _Tetra, _Nb_Nodes, _Nb_tetra, m_iso);
    }
    
    void FitToTSDF(float *** tsdf_grid, my_int3 dim_grid, my_float3 center, float res, float iso = 0.0f, int outerloop_maxiter = 20, int innerloop_surf_maxiter = 5, int innerloop_inner_maxiter = 5, float alpha = 0.01f, float delta = 0.01f, float delta_elastic = 0.01f) {
            ...
            if (OuterIter > -1) {
                for (int s = 0; s < _Nb_Surface; s++) {
                    ...
                    if (dot(grad, normal) < 0.0f)
                        vertex = vertex + grad * delta - normal * (norm(grad) + 0.1f) * alpha_curr;
                    else
                        vertex = vertex + grad * delta + normal * (norm(grad) - 0.1f) * alpha_curr;
    ...
    void Smooth(int innerloop_maxiter = 1) {
        // Copy of vertex position
        float* nodes_copy;
        nodes_copy = new float[3 * _Nb_Nodes];
        memcpy(nodes_copy, _Nodes, 3 * _Nb_Nodes * sizeof(float));
    ...
    void LoadWeights(string path) {
        ...
        ifstream SWfile(path + "weights_outershell.bin", ios::binary);
        if (!SWfile.is_open()) {
        ...
        SWfile.read((char*)_SkinWeights, 24 * _Nb_Nodes * sizeof(float));
        SWfile.close();
    }
    ...
    void ComputeWeights(string path) {
        ...
        int size = 24 * _Nb_Nodes;
        file.write((char*)_SkinWeights, size * sizeof(float));
        file.close();
    ...
    void ComputeWeightsFromSkel(string path) {
        // load kinematic table
        ifstream KTfile(path + "kintree.bin", ios::binary);
        if (!KTfile.is_open()) {
        ...
        /*
        for (int h = 0; h < 24; h++) {    //debug kintree
            cout << h << " : " << kinTree_Table[2*h] << ","  << kinTree_Table[2 * h+1] << endl;
        }*/

        Eigen::Matrix4f* Joints = new Eigen::Matrix4f[24]();
        LoadStarSkeleton(path + "Tshapecoarsejoints.bin", Joints, kinTree_Table, 0.5f);
        ...
        //SavePCasPLY(path + string("skeleton.ply"), skeletonPC, 24);

        _SkinWeights = new float[24 * _Nb_Nodes];
        float* SkinWeights_tmp = new float[24 * _Nb_Nodes];
        // Compute the skinning weight to joints for each voxel
        cout << "SkinWeightsfromJointsDist" << endl;
        std::vector< std::thread > my_threads;
        for (int i = 0; i < 101; i++) {
            my_threads.push_back(std::thread(SkinWeightsfromJointsDistInnerLoop, SkinWeights_tmp, i * (_Nb_Nodes / 100), skeletonPC, _Nb_Nodes, _Nodes, kinTree_Table));
        }
        std::for_each(my_threads.begin(), my_threads.end(), std::mem_fn(&std::thread::join));

        float* Sum_SkinWeights = new float[24 * _Nb_Nodes];
        for (int i = 0; i < _Nb_Nodes * 24; i++) {
            Sum_SkinWeights[i] = SkinWeights_tmp[i];
        }
        int* len_SkinWeights = new int[_Nb_Nodes];
        for (int i = 0; i < _Nb_Nodes; i++) {
            len_SkinWeights[i] = 1;
        }

        int loop = 20;          //if loop == 0 -> no use SumNeighborLoop 
        for (int l = 0; l < loop; l++) {

            cout << "SumNeighborLoop : " << l << endl;
            std::vector< std::thread > my_threads2;
            //vector<vector<int>> Neighbor_nodes(_Nb_Nodes);

            SkinWeights_tmp = Sum_SkinWeights;
            Sum_SkinWeights = new float[24 * _Nb_Nodes];

            for (int i = 0; i < 101; i++) {
                my_threads2.push_back(std::thread(SumNeighborTetraInnerLoop, Sum_SkinWeights, len_SkinWeights, SkinWeights_tmp, i * (_Nb_Nodes / 100), _Nb_Nodes, _Nodes, _Nb_tetra, _Tetra));
            }
            std::for_each(my_threads2.begin(), my_threads2.end(), std::mem_fn(&std::thread::join));

            //avg NeighborTetra nodes
            /*
            for (int i = 0; i < _Nb_Nodes; i++) {
                for (int j = 0; j < 24; j++) {
                    if (Sum_SkinWeights[24 * i + j] != 0.0f) {
                        _SkinWeights[24 * i + j] = Sum_SkinWeights[24 * i + j] / len_SkinWeights[i];
                    }
                    else {
                        _SkinWeights[24 * i + j] = 0.0f;
                    }
                }
            }
            */
        ...
        for (int i = 0; i < _Nb_Nodes * 24; i++) {
            _SkinWeights[i] = Sum_SkinWeights[i];
        }
        delete[] Sum_SkinWeights;

        //normalize with 24joints
        float tmp_sum;
        for (int i = 0; i < _Nb_Nodes; i++) {
            tmp_sum = 0.0f;
            for (int j = 0; j < 24; j++) {
                tmp_sum += _SkinWeights[24 * i + j];
            }
            for (int j = 0; j < 24; j++) {
                if (_SkinWeights[24 * i + j] != 0.0f) {
                    _SkinWeights[24 * i + j] = _SkinWeights[24 * i + j] / tmp_sum;
                }
                //cout << "s_w_" << 24 * i << "_" << j << " : " << _SkinWeights[24 * i + j] << endl;    //debug
            }
        }


        /*
        for (int j = 0; j < 235296; j++) { //debug
            cout << "j:" << j << endl;
            for (int i = 0; i < 24; i++) {
                cout << _SkinWeights[24*j + i] << endl;
            }
        }
        */
    ...
    void ComputeWeightsFromSmplNearVoxel(string path) {
        // Load SMPL model
        float* vertices;
        float* normals_face;
        int* faces;
        ...
        float* SkinWeights_tmp = new float[24 * _Nb_Nodes];
        bool* SkinWeights_valid = new bool[_Nb_Nodes];
        _SkinWeights = new float[24 * _Nb_Nodes];
        // Compute the signed distance to the mesh for each voxel
        std::vector< std::thread > my_threads;
        float thre_dist = 0.01f;
        for (int i = 0; i < 101; i++) {
            my_threads.push_back(std::thread(SkinWeightsFromSmplNearVoxelInnerLoop, SkinWeights_tmp, SkinWeights_valid, i * (_Nb_Nodes / 100), vertices, skin_weights, faces, normals_face, _Nb_Nodes, res[1], _Nodes, thre_dist));
        }
        std::for_each(my_threads.begin(), my_threads.end(), std::mem_fn(&std::thread::join));

        size_t valid_num;
        valid_num = count(SkinWeights_valid, SkinWeights_valid + sizeof(*SkinWeights_valid) * _Nb_Nodes, true);
        cout << "valid/total : " << valid_num << "/" << _Nb_Nodes << endl;

        float* Sum_SkinWeights = new float[24 * _Nb_Nodes];
        bool* SkinWeights_valid_tmp = new bool[_Nb_Nodes];
        for (int i = 0; i < _Nb_Nodes * 24; i++) {
            Sum_SkinWeights[i] = SkinWeights_tmp[i];
        }
        int* len_SkinWeights = new int[_Nb_Nodes];
        for (int i = 0; i < _Nb_Nodes; i++) {
            len_SkinWeights[i] = 0;
            SkinWeights_valid_tmp[i] = SkinWeights_valid[i];
        }

        //Interpolate vertex[valid = false] with tetravoxel;
        int Itploop = 20;
        for (int l = 0; l < Itploop; l++) {
            cout << "InterpolateLoop : " << l << endl;
            std::vector< std::thread > my_threads2;
            //vector<vector<int>> Neighbor_nodes(_Nb_Nodes);

            for (int i = 0; i < _Nb_Nodes * 24; i++) {
                SkinWeights_tmp[i] = Sum_SkinWeights[i];
            }
            for (int i = 0; i < _Nb_Nodes; i++) {
                SkinWeights_valid = SkinWeights_valid_tmp;
            }
            Sum_SkinWeights = new float[24 * _Nb_Nodes];
            SkinWeights_valid_tmp = new bool[_Nb_Nodes];

            for (int i = 0; i < 101; i++) {
                my_threads2.push_back(std::thread(SumNeighborTetraValidInnerLoop, Sum_SkinWeights, SkinWeights_valid_tmp, len_SkinWeights, SkinWeights_tmp, i * (_Nb_Nodes / 100), _Nb_Nodes, _Nodes, _Nb_tetra, _Tetra, SkinWeights_valid));
            }
            std::for_each(my_threads2.begin(), my_threads2.end(), std::mem_fn(&std::thread::join));

            valid_num = count(SkinWeights_valid_tmp, SkinWeights_valid_tmp + sizeof(*SkinWeights_valid_tmp) * _Nb_Nodes, true);
            cout << "valid/total : " << valid_num << "/" << _Nb_Nodes << endl;
            if (valid_num == _Nb_Nodes) {
                break;
            }
        }

        int Sumloop = 20;          //if loop == 0 -> no use SumNeighborLoop 
        for (int l = 0; l < Sumloop; l++) {
            cout << "SumNeighborLoop : " << l << endl;
            std::vector< std::thread > my_threads3;
            //vector<vector<int>> Neighbor_nodes(_Nb_Nodes);
            ...
            for (int i = 0; i < 101; i++) {
                my_threads3.push_back(std::thread(SumNeighborTetraInnerLoop, Sum_SkinWeights, len_SkinWeights, SkinWeights_tmp, i * (_Nb_Nodes / 100), _Nb_Nodes, _Nodes, _Nb_tetra, _Tetra));
            }
            std::for_each(my_threads3.begin(), my_threads3.end(), std::mem_fn(&std::thread::join));
        }
    ...
    void Skin(DualQuaternion *Skeleton) {
        for (int n = 0; n < _Nb_Nodes; n++) {
            _Nodes[3 * n] = _Nodes[3 * n] * 1.1f;
            _Nodes[3 * n + 1] = _Nodes[3 * n+1] * 1.1f;
            _Nodes[3 * n + 2] = _Nodes[3 * n+2] * 1.1f;
            continue;
    ...
    void SkinLBS(Eigen::Matrix4f* Skeleton) {
        for (int n = 0; n < _Nb_Nodes; n++) {
            // Blend the transformations with iterative algorithm
            Eigen::Vector4f point;
            point << _Nodes[3 * n], _Nodes[3 * n + 1], _Nodes[3 * n + 2], 1.0f;
            Eigen::Vector4f vtx;
            ...
            for (int j = 0; j < 24; j++) {
                if (_SkinWeights[24 * n + j] > 0.0) {
                    Transfo = Transfo + _SkinWeights[24 * n + j] * Skeleton[j];
                    sum_weights += _SkinWeights[24 * n + j];
                }
            }
...
int *TetraFromGrid(float ***sdf, float **nodes_out, int **tetra_out, float iso, my_int3 size_grid, my_float3 center_grid, float res) {
    ...
    for (vector<my_float3>::iterator it = Nodes.begin(); it != Nodes.end(); it++) {
        cout << 100.0f * float(count_curr) / float(Nodes.size()) << "%\r";
