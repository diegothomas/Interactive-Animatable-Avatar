...
void SaveMeshToPLY(string path, float* Vertices, float* Normals, unsigned char* Colors, int* Faces, int nbPoints, int nbFaces) {
    ofstream plyfile(path);
    cout << "Saving the mesh to ply" << endl;
    if (plyfile.is_open()) {
        plyfile << "ply" << endl;
        ...
        if (Colors == NULL) {
            for (int i = 0; i < nbPoints; i++) {
                plyfile << Vertices[3 * i] << " " << Vertices[3 * i + 1] << " " << Vertices[3 * i + 2] << " " << Normals[3 * i] << " " << Normals[3 * i + 1] << " " << Normals[3 * i + 2] << " " << 255 << " " << 255 << " " << 255 << endl;
            }
        }
        else {
            for (int i = 0; i < nbPoints; i++) {
                plyfile << Vertices[3 * i] << " " << Vertices[3 * i + 1] << " " << Vertices[3 * i + 2] << " " << Normals[3 * i] << " " << Normals[3 * i + 1] << " " << Normals[3 * i + 2] << " " << int(Colors[3 * i]) << " " << int(Colors[3 * i + 1]) << " " << int(Colors[3 * i + 2]) << endl;
            }
        }
...
void SaveMeshToPLYColor(string path, float* Vertices, float* Normals, int* Faces, unsigned char* Colors, int nbPoints, int nbFaces) {
    ofstream plyfile(path);
    cout << "Saving the mesh to ply" << endl;
    if (plyfile.is_open()) {
        plyfile << "ply" << endl;
        ...
        for (int i = 0; i < nbPoints; i++) {
            //cout << +Colors[3 * i] << " " << +Colors[3 * i + 1] << " " << +Colors[3 * i + 2] << endl;
            plyfile << Vertices[3 * i] << " " << Vertices[3 * i + 1] << " " << Vertices[3 * i + 2] <<  " " << Normals[3 * i] << " " << Normals[3 * i + 1] << " " << Normals[3 * i + 2] << " " << +Colors[3 * i] << " " << +Colors[3 * i + 1] << " " << +Colors[3 * i + 2] << endl;
        }
        //cout << "points written" << endl;
        for (int f = 0; f < nbFaces; f++) {
            plyfile << 3 << " " << Faces[3 * f] << " " << Faces[3 * f + 1] << " " << Faces[3 * f + 2] << endl;
...
void SaveColoredMeshToPLY(string path, float* Vertices, int* Faces, int nbPoints, int nbFaces) {
    ofstream plyfile(path);
    std::cout << "Saving the mesh to ply" << std::endl;
    if (plyfile.is_open()) {
        plyfile << "ply" << endl;
        plyfile << "format ascii 1.0" << endl;
        plyfile << "comment ply file created by Diego Thomas" << endl;
        ...
        for (int i = 0; i < nbPoints; i++) {
            plyfile << Vertices[3 * i] << " " << Vertices[3 * i + 1] << " " << Vertices[3 * i + 2] << " " << int(255.0f * float(i)/float(nbPoints)) << " " << 0 << " " << 0 << endl;
        }
        //cout << "points written" << endl;
        for (int f = 0; f < nbFaces; f++) {
            plyfile << 3 << " " << Faces[3 * f] << " " << Faces[3 * f + 1] << " " << Faces[3 * f + 2] << endl;
...
void SaveTetraMeshToPLY(string path, float *Nodes, float *Normals, int *Faces, int *Tetra, int nbNodes, int nbTetra, int nbFaces, float res, float angle = 0.5f) {
    ofstream plyfile (path);
    cout<<"Saving the mesh to ply"<<endl;
    if (plyfile.is_open()) {
        plyfile << "ply" << endl;
        ...
        plyfile << "property list uchar int vertex_index" << endl;
        plyfile << "element res " << int(res*1000.0f) << endl;
        plyfile << "element angle " << angle << endl;
        plyfile << "end_header" << endl;
        //property float nx\nproperty float ny\nproperty float nz\nelement face \(nbFaces) \nproperty list uchar int vertex_index \nend_header\n" << endl;
...
int* LoadPLY_Mesh_Color(string path, float** Vertices, float** Normals, int** Faces , unsigned char** Colors) {
    int* result = new int[2];
    result[0] = -1; result[1] = -1;
    ...
    if (plyfile.is_open()) {
        ...
        while (words[0].compare(string("end_header")) != 0) {
            ...
            else if (words[0].compare(string("property")) == 0) {
                if (words[2].compare(string("x")) == 0) {
                    vtx_prop = property_cnt;
                    property_cnt++;
                }else if(words[2].compare(string("nx")) == 0) {
                    nml_prop = property_cnt;
                    property_cnt++;
                }else if (words[2].compare(string("red")) == 0) {
                    rgb_prop = property_cnt;
                    property_cnt++;
                }
            }
        ...
        //cout << vtx_prop << " " << nml_prop << " " << rgb_prop << endl;
        ...
        *Colors = new unsigned char[3 * result[0]]();
        unsigned char* Buff_color = *Colors;
        ...
        for (int i = 0; i < result[0]; i++) {
            ...
            std::vector<std::string> words((std::istream_iterator<std::string>(iss)), std::istream_iterator<std::string>());
            if (vtx_prop != 99) {
                Buff_vtx[3 * i]     = std::stof(words[0 + 3 * vtx_prop]);
                Buff_vtx[3 * i + 1] = std::stof(words[1 + 3 * vtx_prop]);
                Buff_vtx[3 * i + 2] = std::stof(words[2 + 3 * vtx_prop]);
            }
            /*
            if (nml_prop != 99) {
                Buff_nmls[3 * i]     = std::stof(words[0 + 3 * nml_prop]);
                Buff_nmls[3 * i + 1] = std::stof(words[1 + 3 * nml_prop]);
                Buff_nmls[3 * i + 2] = std::stof(words[2 + 3 * nml_prop]);
            }
            */
            if (rgb_prop != 99) {
                Buff_color[3 * i]     = std::stof(words[0 + 3 * rgb_prop]);
                Buff_color[3 * i + 1] = std::stof(words[1 + 3 * rgb_prop]);
                Buff_color[3 * i + 2] = std::stof(words[2 + 3 * rgb_prop]);
            }

            //cout << Buff_vtx[3*i] << ", " << Buff_vtx[3*i+1] << ", " << Buff_vtx[3*i+2] << endl;
            //cout << +Buff_color[3 * i] << ", " << +Buff_color[3 * i + 1] << ", " << +Buff_color[3 * i + 2] << endl;
        }
...
int* LoadOBJ_Mesh_uv(string path, float** Vertices, float** Normals, float** UV, int** Faces, int** Faces_UV) {
    int* result = new int[3];
    result[0] = -1; result[1] = -1; result[2] = -1;
    string line;
    ifstream objfile(path, ios::binary);
    ...
    if (objfile.is_open()) {
        ...
        /*
        cout << "vertices[0]: " << vertices[0]  << "vertices[1]: " << vertices[1] << "vertices[2]: " << vertices[2] << endl;
        cout << "uvs[0]: " << uvs[0] << "uvs[1]: " << uvs[1] << endl;
        cout << "faces[0]: " << faces[0] << "faces[1]: " << faces[1] << "faces[2]: " << faces[2] << endl;
        */

        result[0] = vertices.size() / 3;
        result[1] = uvs.size() / 2;
        result[2] = faces.size() / 3;

        // Allocate data
        *Vertices = new float[3 * result[0]]();
        float* Buff_vtx = *Vertices;
        memcpy(Buff_vtx, vertices.data(), 3 * result[0] * sizeof(float));
        *Normals = new float[3 * result[2]]();
        float* Buff_nmls = *Normals;
        *Faces = new int[3 * result[2]]();
        int* Buff_faces = *Faces;
        memcpy(Buff_faces, faces.data(), 3 * result[2] * sizeof(int));
        *UV = new float[2 * result[1]]();
        float* Buff_UV = *UV;
        memcpy(Buff_UV, uvs.data(), 2 * result[1] * sizeof(float));
        *Faces_UV = new int[3 * result[2]]();
        int* Buff_Faces_UV = *Faces_UV;
        memcpy(Buff_Faces_UV, faces_uv.data(), 3 * result[2] * sizeof(int));
        cout << "Computing normals" << endl;
...
int* LoadOBJ_Mesh_retopo(string path, float** Vertices, float** Normals, int** Faces) {
    int* result = new int[2];
    result[0] = -1; result[1] = -1;
    ...
    if (objfile.is_open()) {
        ...
        while (1) {
            ...
            if (word.compare(string("#")) == 0) {
                eof_cnt++;
                cout << "eof_cnt : " << eof_cnt << endl;
                if (eof_cnt >= 4) {
                    break;
                }
            }

            if (word.compare(string("v")) == 0) {
                objfile >> word;
            ...
            if (word.compare(string("f")) == 0) {
                objfile >> word;
                faces.push_back(std::stof(word) -1 );
                objfile >> word;
                faces.push_back(std::stof(word) -1 );
                objfile >> word;
                faces.push_back(std::stof(word) -1 );
...
void LoadStarSkeleton(string filename, Eigen::Matrix4f *Joints_T, int *KinTree_Table, float angle_star = 0.5f) {
    ...
    if (!sklfile.is_open()) {
        cout << "Could not load skeleton of the model @ " << filename << endl;
        sklfile.close();
        return;
    }
...
void LoadASkeleton(string filename, Eigen::Matrix4f* Joints_T, int* KinTree_Table, float angle_star = 0.5f, float angle_A = 0.8f) {
    float* Joints = new float[24 * 3]();
    ifstream sklfile(filename, ios::binary);
    if (!sklfile.is_open()) {
        cout << "Could not load skeleton of the model" << endl;
    ...
    Angles[3 * 13 + 2] = -angle_A;
    Angles[3 * 14 + 2] = angle_A;
...
void UpdateNormals(float* Vertices, float* Normals, int* Faces, int nbVertices, int nbFaces ,bool* valid_vtx_list) {
    ...
    for (int f = 0; f < nbFaces; f++) {
        if ((valid_vtx_list[Faces[3 * f + 0]] == false) || (valid_vtx_list[Faces[3 * f + 1]] == false) || (valid_vtx_list[Faces[3 * f + 2]] == false)) {
            continue;
        }
        // compute face normal
        my_float3 p1 = make_my_float3(Vertices[3 * Faces[3 * f]], Vertices[3 * Faces[3 * f] + 1], Vertices[3 * Faces[3 * f] + 2]);
...
void SkinMeshDBS(float *Vertices, float *SkinWeights, DualQuaternion *Skeleton, int nbVertices) {
    for (int v = 0; v < nbVertices; v++) {
        if (Vertices[3*v] == 0.0f && Vertices[3*v+1] == 0.0f && Vertices[3*v+2] == 0.0f)
            continue;
...
int *LoadPLY_Tet(string path, float **Nodes, int **Faces, int **Tetra){
    ...
    result[0] = -1; result[1] = -1; result[2] = -1; result[3] = 100;
    ...
    if (plyfile.is_open()) {
        ...
        while (words[0].compare(string("end_header")) != 0) {
            if (words[0].compare(string("element")) == 0) {
                ...
                } else if (words[1].compare(string("res")) == 0) {
                    result[3] = std::stoi(words[2]);
                    cout << "resolution is: " << result[3] << endl;
                }
            }
...
void SkinMeshLBS_old(float* Vertices, float* SkinWeights, Eigen::Matrix4f* Skeleton, int nbVertices) {
    for (int v = 0; v < nbVertices; v++) {
        if (Vertices[3 * v] == 0.0f && Vertices[3 * v + 1] == 0.0f && Vertices[3 * v + 2] == 0.0f)
            continue;
        // Blend the transformations with iterative algorithm
        ...
        Vertices[3 * v] = vtx(0) / sum_weights;
        Vertices[3 * v + 1] = vtx(1) / sum_weights;
        Vertices[3 * v + 2] = vtx(2) / sum_weights;
    }
}
...
void SkinMeshLBS(float* Vertices, float* SkinWeights, Eigen::Matrix4f* Skeleton, int nbVertices, bool* valid_vtx_list, bool inverse = false) {
    for (int v = 0; v < nbVertices; v++) {
        if (Vertices[3 * v] == 0.0f && Vertices[3 * v + 1] == 0.0f && Vertices[3 * v + 2] == 0.0f)
            continue;
