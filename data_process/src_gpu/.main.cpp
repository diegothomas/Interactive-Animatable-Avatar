...
void InnerLoop_get_bary_cod(float* vertices_s, float * normals_s ,  int Nb_Vertices_s, float* vertices_m, float * normals_faces_m ,int * faces_m, int Nb_Faces_m, float* result , int * result_face, int id) {
    ...
    for (int i = id; i < min(Nb_Vertices_s, id + Nb_Vertices_s / 100); i++) {
        ...
        for (int f = 0; f < Nb_Faces_m; f++) {
            ...
            if (curr_dist < min_dist) {
                min_dist = curr_dist;
                //closest_point = faces_m[3*f];
                result_face[i * 3] = faces_m[3 * f];
                result_face[i * 3 + 1] = faces_m[3 * f + 1];
                result_face[i * 3 + 2] = faces_m[3 * f + 2];

                BarycentricCodFromFace3D(corr, p0, p1, p2, p3, n);
            }
        }
...
void InnerLoop_aplly_deform(float* vertices_s, int Nb_Vertices_s, float* vertices_m, int Nb_Faces_m, float* bary_cod, int* correspond_face , float* result, int id) {
    ...
    for (int i = id; i < min(Nb_Vertices_s, id + Nb_Vertices_s / 100); i++) {
        for (int k = 0; k < 3; k++) {
            ...
            result[i * 3 + k] = bary_cod[i * 3] * vertices_m[3 * correspond_face[i * 3] + k] + bary_cod[i * 3 + 1] * vertices_m[3 * correspond_face[i * 3 + 1] + k] + bary_cod[i * 3 + 2] * vertices_m[3 * correspond_face[i * 3 + 2] + k];
        }
    }
    delete[] corr;
}
...
float* GetCorrespondences_barycentric_coordinate(float* vertices_s, float* normals_s, int Nb_Vertices_s, float* vertices_m, float* normals_faces_m, int* faces_m, int Nb_Faces_m , int** result_face) {
    float* result = new float[3 * Nb_Vertices_s]();

    // Allocate data
    *result_face = new int[3 * Nb_Vertices_s]();
    int* res_face = *result_face;
        
    cout << "debug" << endl;
    std::vector< std::thread > my_threads;
    for (int i = 0; i < 101; i++) {
        //InnerLoop2(skin_weights, vertices_s, normals_s, Nb_Vertices_s, vertices_m, normals_faces_m, faces_m, Nb_Faces_m, result, i * (Nb_Vertices_s / 100));
        my_threads.push_back(std::thread(InnerLoop_get_bary_cod, vertices_s, normals_s, Nb_Vertices_s, vertices_m, normals_faces_m, faces_m, Nb_Faces_m, result , res_face, i * (Nb_Vertices_s / 100)));
        cout << "i : " << i << endl;
    }
    std::for_each(my_threads.begin(), my_threads.end(), std::mem_fn(&std::thread::join));
    return result;
...
float* Apply_SMPLdeform(float* vertices_s, int Nb_Vertices_s, float* vertices_m, int Nb_Faces_m , float* bary_cod, int* correspond_face) {
    float* result = new float[3 * Nb_Vertices_s]();

    std::vector< std::thread > my_threads;
    for (int i = 0; i < 101; i++) {
        my_threads.push_back(std::thread(InnerLoop_aplly_deform, vertices_s, Nb_Vertices_s, vertices_m, Nb_Faces_m, bary_cod , correspond_face , result, i * (Nb_Vertices_s / 100)));
        cout << "i : " << i << endl;
    }
    std::for_each(my_threads.begin(), my_threads.end(), std::mem_fn(&std::thread::join));
    return result;
...
void ComputeWeights(string path, float* Vertices_in, int NB_v , string save_path) {
    // Load SMPL model
    float* vertices;
    float* normals_face;
    int* faces;
    ...
    auto file = std::fstream(save_path, std::ios::out | std::ios::binary);
    int size = 24 * NB_v;
...
void SmoothOuterShell(string template_dir, TetraMesh* TetMesh, TetraMesh* TetMeshHD) {
    ...
    Eigen::Matrix4f* Joints_T = new Eigen::Matrix4f[24]();
    LoadStarSkeleton("D:/Data/Human/HUAWEI/Iwamoto/data/smplparams/T_joints.bin", Joints_T, kinTree_Table, 0.5f);
...
void Calc_SMPL_and_Outershell_corresponding(string template_dir) {
    //Load smpl 
    float* vertices;
    float* normals_face;
    int* faces;
    int* res = LoadPLY_Mesh(template_dir + "Template_skin.ply", &vertices, &normals_face, &faces);
    cout << "The PLY file has " << res[0] << " vertices and " << res[1] << " faces" << endl;
    ...
    int* faces_ft;
    int* res_ft = LoadPLY_Mesh(template_dir + "shellb_template.ply", &vertices_ft, &normals_face_ft, &faces_ft);
    cout << "The PLY file has " << res_ft[0] << " vertices and " << res_ft[1] << " faces" << endl;

    float* bary_cod;
    int* result_face;
    bary_cod = GetCorrespondences_barycentric_coordinate(vertices_ft, normals_face_ft, res_ft[0], vertices, normals_face, faces , res[1] , &result_face);

    //save SMPL_and_Outershll_barycentric
    ofstream file(template_dir + "SMPL_and_Outershell_Barcycentric.bin", ios::binary);
    if (file.is_open()) {
        file.write((char*)bary_cod, 3 * res_ft[0] * sizeof(float));
        file.close();
    }

    ofstream facefile(template_dir + "SMPL_and_Outershell_CorrespondFace.bin", ios::binary);
    if (facefile.is_open()) {
        facefile.write((char*)result_face, 3 * res_ft[0] * sizeof(int));
        facefile.close();
    }
}

void SMPLdeformToOutershell(string template_dir , string inoutdir , int idx_file) {
    //Load deformed smpl 
    float* vertices;
    float* normals_face;
    int* faces;
    //int* res = LoadPLY_Mesh(template_dir + "Template_skin.ply", &vertices, &normals_face, &faces);
    //int* res = LoadPLY_Mesh("D:/Project/Human/Avatar-In-The-Shell/SMPL_python_v.1.1.0/smpl/smpl_webuser/hello_world/output/posed_smpl_00372.ply", &vertices, &normals_face, &faces);

    string idx_s = to_string(idx_file);
    idx_s.insert(idx_s.begin(), 4 - idx_s.length(), '0');
    int* res = LoadPLY_Mesh(string(inoutdir) + string("DeformedSMPL/Template_pd_") + idx_s + string(".ply"), &vertices, &normals_face, &faces);
    //int* res = LoadPLY_Mesh(string(inoutdir) + string("data/DeformedSMPL/Template_pd_") + idx_s + string(".ply"), &vertices, &normals_face, &faces);
    //int* res = LoadPLY_Mesh(string(inoutdir) + string("data/DeformedSMPL/Template_pd_shape_") + idx_s + string(".ply"), &vertices, &normals_face, &faces);
    cout << "The PLY file has " << res[0] << " vertices and " << res[1] << " faces" << endl;
    ...
    //Load SMPL_and_Outershll_barycentric
    float* bary_cod = new float[24 * res_ft[0]];
    ifstream file(template_dir + "SMPL_and_Outershell_Barcycentric.bin", ios::binary);
    if (!file.is_open()) {
        cout << "Could not load skin weights" << endl;
        file.close();
        return;
    }
    file.read((char*)bary_cod, 3 * res_ft[0] * sizeof(float));
    file.close();

    //Load SMPL_and_Outershell_CorrespondFace
    int* correspond_face = new int[24 * res_ft[0]];
    ifstream facefile(template_dir + string("SMPL_and_Outershell_CorrespondFace.bin"), ios::binary);
    if (!facefile.is_open()) {
        cout << "Could not load skin weights" << endl;
        facefile.close();
        return;
    }
    facefile.read((char*)correspond_face, 3 * res_ft[0] * sizeof(int));
    facefile.close();

    float* out_vertex;
    out_vertex = Apply_SMPLdeform(vertices_ft, res_ft[0], vertices, res[1] , bary_cod , correspond_face);
    cout << "saving" << endl;
    //SaveMeshToPLY(template_dir + string("Deformded_FittedOutershell.ply"), out_vertex, faces_ft, res_ft[0], res_ft[1]);
    SaveMeshToPLY(string(inoutdir) + string("DeformedSMPL/Shellb_template_pd_") + idx_s + string(".ply"), out_vertex, faces_ft, res_ft[0], res_ft[1]);
    //SaveMeshToPLY(string(inoutdir) + string("data/DeformedSMPL/Shellb_template_pd_") + idx_s + string(".ply"), out_vertex, faces_ft, res_ft[0], res_ft[1]);
    //SaveMeshToPLY(string(inoutdir) + string("data/DeformedSMPL/Shellb_template_pd_shape_") + idx_s + string(".ply"), out_vertex, faces_ft, res_ft[0], res_ft[1]);
}
...
/*
void Save_Outershell_weight(string template_dir) {
    string Weight_mode = "Load";    //"Load" or "Save"
    TetraMesh* TetMesh = new TetraMesh(template_dir, GRID_RES, Weight_mode);
    TetMesh->Save_Surface_weight(template_dir);
    float* vertices;
    float* normals;
    float* weights;
    int* faces;
    int* res;
    res = TetMesh->GetSurface(&vertices, &normals, &weights, &faces);
    SaveMeshToPLY(template_dir + string("Wrong_Surface.ply"), vertices, normals, faces, res[0], res[1]);
}
*/

void Save_shellb_template_weight(string template_dir) {
    cout << "==============Load shellb_template.ply==================" << endl;
    float* vertices_sb;
    float* normals_face_sb;
    int* faces_sb;
    int* res_sb = LoadPLY_Mesh(template_dir + "shellb_template.ply", &vertices_sb, &normals_face_sb, &faces_sb);
    cout << "The PLY file has " << res_sb[0] << " vertices and " << res_sb[1] << " faces" << endl;

    cout << "==============Compute shellb_template weight from template skin==================" << endl;
    cout << "save to " << template_dir + "shellb_template_weight.bin" << endl;
    ComputeWeights(template_dir , vertices_sb , res_sb[0] , template_dir + "weight_shellb_template.bin");
    return;
}
...
void FitTetraToCanonicalFromOut(string output_dir, string input_dir, string template_dir, int idx_file, TetraMesh* TetMesh, TetraMesh* TetMeshHD, bool volume_fit = false, bool use_fixed_weights = false) {
    cout << "==============Build the user specific Outer shell and Fit the outershell to it==================" << endl;
    string idx_s = to_string(idx_file);
    idx_s.insert(idx_s.begin(), 3 - idx_s.length(), '0'); //4
    string idx_s_pls1 = to_string(idx_file);
    ...
    idx_p.insert(idx_p.begin(), 4 - idx_p.length(), '0'); //3
    ...
    float* normals_face_in;
    int* faces_in;
    int* res_in = LoadPLY_Mesh(input_dir + "data/canonical_meshes/mesh_" + idx_s + ".ply", &vertices_in, &normals_face_in, &faces_in);      //datas
    //int* res_in = LoadPLY_Mesh(template_dir + "Template_skin.ply", &vertices_in, &normals_face_in, &faces_in);    //make "shellb_template.ply"
    if (res_in[0] == -1)
        return;
    ...
    // Load skinning weights    
    float* skin_weights = new float[24 * res_in[0]];
    ifstream SWfile(input_dir + "data/canonical_meshes/skin_weights_" + idx_s_pls1 + ".bin", ios::binary);
    if (!SWfile.is_open()) {
        cout << "Could not load skin weights" << endl;
        SWfile.close();
        return;
    ...
    */
    ...
    Eigen::Matrix4f* Joints_T = new Eigen::Matrix4f[24]();
    //LoadStarSkeleton(template_dir + "Tshapecoarsejoints.bin", Joints_T, kinTree_Table, 0.5f);
    LoadStarSkeleton(input_dir + "data/smplparams/T_joints.bin", Joints_T, kinTree_Table, 0.5f);    //neutral→dd-Tpose
    Eigen::Matrix4f* Joints = new Eigen::Matrix4f[24]();
    LoadStarSkeleton(template_dir + "Tshapecoarsejoints.bin", Joints, kinTree_Table, 0.5f);         //neutral→canonical-Tpose
    //LoadStarSkeleton(input_dir + "data/smplparams/T_joints.bin", Joints, kinTree_Table, 0.5f);    //dd-pose→canonical-Tpose
    ...
    for (int s = 0; s < 24; s++) {     //
        ...
        skeletonDQ[s] = DualQuaternion(Joints_T[s] * InverseTransfo(Joints[s]));
    }
    ...
    */
    ...
    float* weights_smpl;
    if (volume_fit) {
        cout << "Volume fit!" << endl;
        
        TetMesh->Skin(skeletonDQ);
        TetMesh->Smooth(10);
        res = TetMesh->GetSurface(&vertices, &normals, &weights, &faces);
        SaveMeshToPLY(output_dir + string("Output_Mesh_b.ply"), vertices, faces, res[0], res[1]);
        ...
        TetMesh->FitToTSDF(sdf, size_grid, center_grid, GRID_RES, 0.0f);//, 20, 1, 10, 0.01f, 0.1f);
        ...
        //TetMesh->Skin(skeletonDQ);
        //TetMesh->Smooth(10);
        //SaveTetraMeshToPLY(output_dir + string("BetaOuter.ply"), TetMesh->Nodes(), TetMesh->Normals(), TetMesh->Faces(), TetMesh->Tetras(), TetMesh->NBNodes(), TetMesh->NBTets(), TetMesh->NBFaces(), TetMesh->Res());

        // Get surface vertices
        //res = TetMesh->GetSurface(&vertices, &normals, &weights, &faces);
        float* normals_face;
        //res = LoadPLY_Mesh(input_dir + "data/smplparams_centered/T-shape.ply", &vertices, &normals_face, &faces);
        //res = LoadPLY_Mesh(template_dir + "shellb_basic.ply", &vertices, &normals_face, &faces);           //make "shellb_template.ply"
        res = LoadPLY_Mesh(template_dir + "shellb.ply", &vertices, &normals_face, &faces);
        //ComputeWeights(template_dir, vertices, res[0]);
        // Scale
        for (int n = 0; n < res[0]; n++) {
            vertices[3 * n] = vertices[3 * n] * 0.95;
            vertices[3 * n + 1] = vertices[3 * n + 1] * 1.00;
            vertices[3 * n + 2] = vertices[3 * n + 2] * 1.05;
        }
        ...
        SaveMeshToPLY(output_dir + string("OS.ply"), vertices, normals, faces, res[0], res[1]);
        //return;

        /*float* normals_face;
        res = LoadPLY_Mesh(input_dir + "data/canonical_meshes/mesh_001.ply", &vertices, &normals_face, &faces);
        UpdateNormals(vertices, normals, faces, res[0], res[1]);*/
        ...
        //SaveTetraMeshToPLY(output_dir + string("BetaOuter.ply"), TetMeshHD->Nodes(), TetMeshHD->Normals(), TetMeshHD->Faces(), TetMeshHD->Tetras(), TetMeshHD->NBNodes(), TetMeshHD->NBTets(), TetMeshHD->NBFaces(), TetMeshHD->Res());
        ...
        SaveMeshToPLY(output_dir + "shellb_template.ply", vertices, normals, faces, res[0], res[1]);      //make "shellb_template.ply"
        return;                                                                                           //make "shellb_template.ply"
    ...
    if (use_fixed_weights) {
        ...
        //res = LoadPLY_Mesh(output_dir + "FittedMesh_" + idx_s + ".ply", &vertices, &normals_face, &faces);
        //normals = new float[3 * res[0]];
        UpdateNormals(vertices, normals, faces, res[0], res[1]);
        ...
        delete[] normals_face_in;
        delete[] faces_in;
        res_in[0] = LoadPLY(input_dir + "data/canonical_meshes/PC_" + idx_s + ".ply", &vertices_in);
        // Load skinning weights    
        delete[] skin_weights;
        skin_weights = new float[24 * res_in[0]];
        ifstream SWfile(input_dir + "data/centered_meshes/skin_weights_" + idx_s_pls1 + ".bin", ios::binary);
        if (!SWfile.is_open()) {
            cout << "Could not load skin weights" << endl;
            SWfile.close();
            return;
        ...
        //SaveMeshToPLY(output_dir + "FittedTest_" + idx_s + ".ply", vertices, normals, faces, res[0], res[1]);
        //SaveMeshToPLY(output_dir + "FittedTest2_" + idx_s + ".ply", vertices_in, vertices_in, faces_in, res_in[0], res_in[1]);
    ...
    LoadSkeleton(input_dir + "data/smplparams/T_joints.bin", input_dir + "data/smplparams/pose_" + idx_s_pls1 + ".bin", Joints, kinTree_Table);

    //Eigen::Matrix4f* skeleton = new Eigen::Matrix4f[24]();
    for (int s = 0; s < 24; s++) {
        //skeleton[s] = Joints[s] * InverseTransfo(Joints_T[s]);      //
        skeleton[s] = Joints_T[s] * InverseTransfo(Joints[s]);    //old_lbs new_lbs(inverse = true) T→pose
    }

    //SaveMeshToPLY(output_dir + "FittedMesh_" + idx_s + ".ply", vertices, normals, NULL, faces, res[0], res[1]);
    ...
    //SaveMeshToPLY(output_dir + "PosedMesh_in_" + idx_s + ".ply", vertices_in, vertices_in, faces_in, res_in[0], res_in[1]);
    //SavePCasPLY(output_dir + "PosedMesh_in_" + idx_s + ".ply", vertices_in, res_in[0]);
    ...
    */
    ...
    int* faces_GT;
    int* faces_uv_GT;
    int* res_GT = LoadOBJ_Mesh_uv(input_dir + "data/centered_meshes/" + "mesh" + idx_s_pls1 + ".obj", &vertices_GT, &normals_face_GT, &uv_GT, &faces_GT, &faces_uv_GT);
    ...
    */
    ...
    //cout << output_dir + "FittedMesh_" + idx_s + ".ply" << endl;
    SaveMeshToPLY(output_dir + "FittedMesh_" + idx_s + ".ply", vertices_cpy, normals_cpy, NULL, faces, res[0], res[1]);
...
void SkinShellbToBasic(string output_dir, string input_dir, string template_dir, TetraMesh* TetMesh, TetraMesh* TetMeshHD, bool volume_fit = false, bool use_fixed_weights = false) {
    string idx_s_pls1 = to_string(1);
    idx_s_pls1.insert(idx_s_pls1.begin(), 4 - idx_s_pls1.length(), '0'); //4 _ +1
    
    cout << "==============Build the user specific Outer shell and Fit the outershell to it==================" << endl;
    cout << "==== Step 1   :  Skinning Outershell to T-Pose ===" << endl;
    cout << "==== Step 1.1 :  Load Outershell and skinning weight====" << endl; 
    float* vertices;
    float* normals_face;
    int* faces;
    int* res;
    res = LoadPLY_Mesh(template_dir + "shellb.ply", &vertices, &normals_face, &faces);
    cout << "shellb.ply file has " << res[0] << " surfaces and " << res[1] << " faces" << endl;

    float* weights_surface;
    weights_surface = new float[24 * res[0]];
    // Load skinning weights    
    ...
    SWfile.read((char*)weights_surface, 24 * res[0] * sizeof(float));
    SWfile.close();

    cout << "==== Step 1.2 : Load pose data ====" << endl;
    // load kinematic table
    ifstream KTfile(template_dir + "kintree.bin", ios::binary);
    if (!KTfile.is_open()) {
        cout << "Could not load kintree" << endl;
    ...
    Eigen::Matrix4f* Joints_T = new Eigen::Matrix4f[24]();
    //LoadStarSkeleton(template_dir + "Tshapecoarsejoints.bin", Joints_T, kinTree_Table, 0.5f);
    LoadStarSkeleton(input_dir + "data/smplparams/T_joints.bin", Joints_T, kinTree_Table, 0.5f);    //T-pose
    Eigen::Matrix4f* Joints = new Eigen::Matrix4f[24]();
    //LoadSkeleton(input_dir + "data/smplparams/T_joints.bin", input_dir + "data/smplparams/pose_" + idx_s_pls1 + ".bin", Joints, kinTree_Table);
    LoadStarSkeleton(template_dir + "Tshapecoarsejoints.bin", Joints, kinTree_Table, 0.5f);         //A-pose?

    DualQuaternion* skeletonDQ = new DualQuaternion[24];
    Eigen::Matrix4f* skeleton = new Eigen::Matrix4f[24]();
    for (int s = 0; s < 24; s++) {     //
        skeleton[s] = Joints_T[s] * InverseTransfo(Joints[s]);                      //T→coarse?
    }
    ...
    SkinMeshLBS(vertices, weights_surface, skeleton, res[0], true);
    UpdateNormals(vertices, normals_face, faces, res[0], res[1]);
    SaveMeshToPLY(output_dir + "shellb_basic.ply", vertices, normals_face, faces, res[0], res[1]);
}

void SkinRetopo(string input_mesh ,string output_dir, string input_dir, string template_dir , int id , int src_id) {
    string idx_s_pls1 = to_string(id);
    idx_s_pls1.insert(idx_s_pls1.begin(), 4 - idx_s_pls1.length(), '0'); //4 _ +1

    string src_idx = to_string(src_id);
    src_idx.insert(src_idx.begin(), 4 - src_idx.length(), '0'); //4 _ +1
    ...
    int* faces;
    int* res;
    //res = LoadOBJ_Mesh("D:/Data/Human/HUAWEI/Meshes-fit-smpl/4dviews_nobi_retopo_0000_centered.obj", &vertices, &normals_face, &faces);
    //res = LoadPLY_Mesh(template_dir + "shellb.ply", &vertices, &normals_face, &faces);
    //res = LoadOBJ_Mesh(input_mesh, &vertices, &normals_face, &faces);
    res = LoadPLY_Mesh(input_mesh, &vertices, &normals_face, &faces);
    cout << "mesh file has " << res[0] << " surfaces and " << res[1] << " faces" << endl;
    ...
    //ifstream SWfile(template_dir + "weights_surface.bin", ios::binary);
    //ifstream SWfile(template_dir + "TshapeWeights.bin", ios::binary);
    ifstream SWfile("D:/Data/Human/HUAWEI/MIT/MIT/data/centered_meshes/skin_weights_0000.bin", ios::binary);
    if (!SWfile.is_open()) {
        cout << "Could not load skin weights" << endl;
        SWfile.close();
        return;
    ...
    Eigen::Matrix4f* Joints_T = new Eigen::Matrix4f[24]();
    //LoadStarSkeleton(template_dir + "Tshapecoarsejoints.bin", Joints_T, kinTree_Table, 0.5f);
    LoadStarSkeleton(input_dir + "data/smplparams/T_joints.bin", Joints_T, kinTree_Table, 0.5f);    //data depend T-pose
    Eigen::Matrix4f* Joints = new Eigen::Matrix4f[24]();
    //LoadSkeleton(input_dir + "data/smplparams/T_joints.bin", input_dir + "data/smplparams/pose_" + idx_s_pls1 + ".bin", Joints, kinTree_Table);
    //LoadStarSkeleton(template_dir + "Tshapecoarsejoints.bin", Joints, kinTree_Table, 0.5f);         //A-pose?

    LoadSkeleton(input_dir + "data/smplparams/T_joints.bin", input_dir + "data/smplparams/pose_" + src_idx + ".bin", Joints, kinTree_Table);
    ...
    //SkinMeshLBS(vertices, weights_surface, skeleton, res[0], true);    //T-pose → 0000
    SkinMeshLBS(vertices, weights_surface, skeleton, res[0], false);    //0000 → T-pose
    ...
    SkinMeshLBS(vertices, weights_surface, skeleton, res[0], true);    // T-pose → xxxx
    UpdateNormals(vertices, normals_face, faces, res[0], res[1]);
    SaveMeshToPLY(output_dir + "mesh_" + idx_s_pls1 + ".ply", vertices, normals_face, faces, res[0], res[1]);
}
...
void FitSMPLToCanonical(string output_dir, string input_dir, string template_dir, int idx_file, TetraMesh* TetMeshHD) {
    ...
    string idx_s_pls1 = to_string(idx_file);
    idx_s_pls1.insert(idx_s_pls1.begin(), 4 - idx_s_pls1.length(), '0'); 
    string idx_p = to_string(idx_file);
...
int main( int argc, char **argv )
{
    ...
    /*
    SkinShellbToBasic("D:/Data/Human/HUAWEI/Iwamoto/data/FittedMesh_pose0517/"
        , "D:/Data/Human/HUAWEI/Iwamoto/"
        , "D:/Data/Human/Template-star-0.015-0.05/"
        ,  TetMesh, TetMeshHD, false, false);
    */

    /*
    for (int i = 0; i < 175; i++) {
        SkinRetopo(
            "D:/Data/Human/HUAWEI/MIT/MIT/data/centered_meshes/mesh_0000.ply",
            "D:/Data/Human/HUAWEI/MIT/MIT/data/Posed_retopo/"
            ,"D:/Data/Human/HUAWEI/MIT/MIT/"
            ,"D:/Data/Human/Template-star-0.015-0.05/"
            ,i , 0);
    }
    return 0;
    */
    ...
    //Debug
    /*
    TetraMesh* TetMesh = new TetraMesh("D:/Data/Human/Template-star-0.015-0.05/", GRID_RES);
    //TetraMesh* TetMesh = new TetraMesh("D:/Human_data/Template-star-0.015/", GRID_RES);
    //TetMesh->Skin((DualQuaternion*)NULL);
    TetraMesh* TetMeshHD = new TetraMesh("D:/Data/Human/Template-star-0.008/", GRID_RES);
    TetMeshHD->Skin((DualQuaternion*)NULL);
    SmoothOuterShell("D:/Data/Human/Template-star-0.015-0.05/", TetMesh, TetMeshHD);
    return 0 ;
    */

    //Save_shellb_template_weight("D:/Data/Human/Template-star-0.015-0.05/");
    //return 0;

    //Calculate barycentric coordinate between SMPL Faces and Outershell vertices
    //Calc_SMPL_and_Outershell_corresponding("D:/Data/Human/Template-star-0.015-0.05/");
      
    //Deform Fitted Outershell #deform smpl for training (comment out (0921)) ?
    /*
    for (int i = 0; i < 639; i++) {
        if (i % 2 == 0){
            cout << "data : " + to_string(i) << endl;
            SMPLdeformToOutershell("D:/Data/Human/Template-star-0.015-0.05/", "D:/Data/Human/HUAWEI/test_motion_list/motion_lists/gLO_sBM_cAll_d14_mLO1_ch05/", i);
        }
    }
    */
    ...
    for (int i = 1; i < 458; i++) {
        cout << "data : " + to_string(i) << endl;
        SMPLdeformToOutershell("D:/Data/Human/Template-star-0.015-0.05/", "D:/Data/Human/HUAWEI/test_motion_list/motion_lists/Dancing_Twerk/", i);
    }

    for (int i = 1; i < 58; i++) {
        cout << "data : " + to_string(i) << endl;
        SMPLdeformToOutershell("D:/Data/Human/Template-star-0.015-0.05/", "D:/Data/Human/HUAWEI/test_motion_list/motion_lists/Joyful_Jump/", i);
    }

    for (int i = 1; i < 152; i++) {
        cout << "data : " + to_string(i) << endl;
        SMPLdeformToOutershell("D:/Data/Human/Template-star-0.015-0.05/", "D:/Data/Human/HUAWEI/test_motion_list/motion_lists/Shooting_Arrow/", i);
    }

    for (int i = 1; i < 86; i++) {
        cout << "data : " + to_string(i) << endl;
        SMPLdeformToOutershell("D:/Data/Human/Template-star-0.015-0.05/", "D:/Data/Human/HUAWEI/test_motion_list/motion_lists/Standing_Taunt_Battlecry/", i);
    }

    for (int i = 1; i < 117; i++) {
        cout << "data : " + to_string(i) << endl;
        SMPLdeformToOutershell("D:/Data/Human/Template-star-0.015-0.05/", "D:/Data/Human/HUAWEI/test_motion_list/motion_lists/Taunt/", i);
    }
    return 0;
    */

    /**
            0. Load the outer shells
     */
    TetraMesh* TetMesh = new TetraMesh("Z:/Human/b20-kitamura/Human_datasets/Template-star-0.015-0.05/", GRID_RES);
    //TetraMesh* TetMesh = new TetraMesh("D:/Human_data/Template-star-0.015/", GRID_RES);
    //TetMesh->Skin((DualQuaternion*)NULL);
    TetraMesh* TetMeshHD = new TetraMesh("Z:/Human/b20-kitamura/Human_datasets/Template-star-0.008/", GRID_RES);
    //TetMeshHD->Skin((DualQuaternion*)NULL);   //if body == big → comment out

    //SmoothOuterShell("D:/Human_data/Template-star-0.015-0.05", TetMesh, TetMeshHD);
    //return 0;
    ...
    for (int i = 1; i < 24; i++) {
        ...
        // Datasets
        FitTetraToCanonicalFromOut("Z:/Human/b20-kitamura/Human_datasets/CAPE-33/data/FittedMesh_pose/"
            , "Z:/Human/b20-kitamura/Human_datasets/CAPE-33/"
            , "Z:/Human/b20-kitamura/Human_datasets/Template-star-0.015-0.05/"
            , i, TetMesh, TetMeshHD, false, false);
    }
    return 0;
    */
    ...
    for (int i = 1; i < 2; i++) {
        cout << "data : " + to_string(i) << endl;
        /*FitSMPLToCanonicalCubic("D:/Human_data/4D_data/data/Fitted_smpl_rep/"
            , "D:/Human_data/4D_data/"
            , "D:/Human_data/Template-star-0.015-0.05/"
            , i);*/
        /*/FitSMPLToCanonical("D:/Human_data/4D_data/data/Fitted_smpl_rep/"
            , "D:/Human_data/4D_data/"
            , "D:/Human_data/Template-star-0.015-0.05/"
            , i, TetMeshHD);*/
        ...
        // Smpl
        /*
        FitTetraToCanonicalFromOut("D:/Human_data/TMP/"
            , "D:/Human_data/4D_data/"
            , "D:/Human_data/Template-star-0.015-0.05/"
            , i, TetMesh, TetMeshHD, false, false);
        */
        ...
        // Datasets
        FitTetraToCanonicalFromOut("Z:/Human/b20-kitamura/Human_datasets/4D-people/rp_regina_4d_001_walking_BLD_test2/data/FittedMesh_pose/"
            , "Z:/Human/b20-kitamura/Human_datasets/4D-people/rp_regina_4d_001_walking_BLD_test2/"
            , "Z:/Human/b20-kitamura/Human_datasets/Template-star-0.015-0.05/"
            , i, TetMesh, TetMeshHD, false, false);
    }
    ...
    for (int i = 1; i < 24; i++) {
        ...
        // Datasets
        FitTetraToCanonicalFromOut("D:/Data/Human/HUAWEI/Female_ballet_dancer/data/FittedMesh_pose/"
            , "D:/Data/Human/HUAWEI/Female_ballet_dancer/ "
            , "D:/Data/Human/Template-star-0.015-0.05/"
            , i, TetMesh, TetMeshHD, false, false);
    }
    ...
    for (int i = 0; i < 17; i++) {
        ...
        // Datasets
        FitTetraToCanonicalFromOut("D:/Data/Human/HUAWEI/Male_03_Ref_Bundle/data/FittedMesh_pose/"
            , "D:/Data/Human/HUAWEI/Male_03_Ref_Bundle/ "
            , "D:/Data/Human/Template-star-0.015-0.05/"
            , i, TetMesh, TetMeshHD, false, false);
    }
    ...
    for (int i = 0; i < 7; i++) {
        ...
        // Datasets
        FitTetraToCanonicalFromOut("D:/Data/Human/HUAWEI/range_of_movement/data/FittedMesh_pose/"
            , "D:/Data/Human/HUAWEI/range_of_movement/ "
            , "D:/Data/Human/Template-star-0.015-0.05/"
            , i, TetMesh, TetMeshHD, false, false);
    }
