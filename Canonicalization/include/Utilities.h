...
//#define MODEL_NAME string("6996777") // -> Modify _max_nbF!!
#define MODEL_NAME string("1460314")

#ifdef _APPLE_
#define PATH_DATA string("/Users/diegothomas/Documents/Projects/GA/Data/")
#else
...
void display(float * A, int N);
...
#ifndef _CUDA_
    typedef struct my_int2 {
        int x;
        int y;
        
        // Initializer
        my_int2(): x(0), y(0) {}
        my_int2(int a, int b): x(a), y(b) {}
    } my_int2;
    ...
    typedef struct my_int4 {
        ...
        // Initializer
        my_int4(): x(0), y(0), z(0), w(0) {}
        my_int4(int a, int b, int c, int d): x(a), y(b), z(c), w(d) {}
    } my_int4;
...
//    extend float {
//        static float operator* my_float3(v) {
//            return my_float3(s*v.x, s*v.y, s*v.z);
//        }
//    }

    typedef struct my_float4 {
        float x;
        float y;
        float z;
        float w;
        
        my_float4(): x(0.0f), y(0.0f), z(0.0f), w(0.0f) {}
        my_float4(float X, float Y, float Z, float W): x(X), y(Y), z(Z), w(W) {}
                
        // Addition
        my_float4 operator+ (my_float4 a) {
            return my_float4(a.x + x, a.y + y, a.z + z, a.w + w);
        }
    } my_float4;
...
float IsInsideTriangle3D(my_float3 p0, my_float3 p1, my_float3 p2, my_float3 p3, my_float3 n);

float DistancePointFace3D(my_float3 p0, my_float3 p1, my_float3 p2, my_float3 p3, my_float3 n, bool approx = false);

my_float3 ProjPointFace3D(my_float3 p0, my_float3 p1, my_float3 p2, my_float3 p3, my_float3 n);
