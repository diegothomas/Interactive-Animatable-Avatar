    ...
    with open(filename, 'r') as fin:
        ...
        while read_header:
            ...
            if line_s[0] == 'element':
                ...
                read_header = False
        vertices = np.ndarray((num_vertex, 3))
        for i in range(num_vertex):
            line = fin.readline()
            line_s = line.split(' ')
            vertices[i,:] = [float(line_s[0]), float(line_s[1]), float(line_s[2])]
        faces = np.ndarray((num_face, 3), dtype=np.int)
    ...
    with open(filename, 'r') as fin:
        ...
        while read_header:
            ...
            if line_s[0] == 'element':
                ...
                read_header = False
        vertices = np.ndarray((num_vertex, 3))
        normales = np.ndarray((num_vertex, 3))
        for i in range(num_vertex):
            line = fin.readline()
            line_s = line.split(' ')
            vertices[i,:] = [float(line_s[0]), float(line_s[1]), float(line_s[2])]
            normales[i,:] = [float(line_s[3]), float(line_s[4]), float(line_s[5])]
        faces = np.ndarray((num_face, 3), dtype=np.int)
        for i in range(num_face):
            line = fin.readline()
            line_s = line.split(' ')
            faces[i, :] = [int(line_s[1]), int(line_s[2]), int(line_s[3])]
        return vertices, normales, faces
...
def save_ply_nmle(fname, v, nmle, f=None, col=None, lab=None, fcol=None):
    cname = ('gray', 'red', 'blue', 'yellow', 'magenta', 'green', 'indigo', 'darkorange', 'cyan', 'pink', 'yellowgreen')
    cd = skimage.color.color_dict
    cols1 = [np.array(cd[cn]) * 255 for cn in cname]
    cols2 = [np.array(cd[cn]) * 127 for cn in cname]
    ...
    with open(fname, 'w') as fout:
        ...
        for i in range(n):
            ...
            fout.write('%f %f %f %f %f %f %.0f %.0f %.0f\n' % (v[0, i], v[1, i], v[2, i], nmle[0, i], nmle[1, i], nmle[2, i], c[0], c[1], c[2]))
