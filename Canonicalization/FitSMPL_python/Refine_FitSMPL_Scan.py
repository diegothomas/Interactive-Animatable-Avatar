...
def FitSMPL(init_pose_path, scanpath, outputpath, tmppath, verbose, idx , opt_target , init_flg  , init_trans_path = None , m = None , first_flg = False):
    ...
    for it in range(itr):
        ...
        for i in range(V.shape[0]):
            ...
            indices_cross_corr[i] = id
            cross_weights_corr[i] = w
...
if __name__ == "__main__":
    ...
    if init_flg == True:
        ...
        init_pose_path     = os.path.join(path , "data/smplparams/pose_"  + str(prev_id).zfill(4) + "_init.bin")    #copied from blender
        init_trans_path    = os.path.join(path , "data/smplparams/trans_" + str(prev_id).zfill(4) + "_init.bin")    #copied from blender
        tmp_path           = os.path.join(path , "tmp/")
        output_path        = os.path.join(path , "data/smplparams/pose_"  + str(idx).zfill(4) + ".bin" )

        if args.resampled_flg == "True":
            scan_path          = os.path.join(path , "data/resampled_meshes/" + str(idx).zfill(4) + ".obj" )
        elif args.resampled_flg == "False":
            scan_path          = os.path.join(path , "data/src_meshes/"  + str(idx).zfill(4) + ".obj"      )
        ...
        for idx in tqdm.tqdm(range(start_id,args.size)):
            tmp_path       = os.path.join(path , "tmp/")
            init_pose_path     = os.path.join(path , "data/smplparams/pose_" + str(prev_id).zfill(4) + ".binfitted_model.pkl") #output of first process(init = True)
            output_path        = os.path.join(path , "data/smplparams/pose_" + str(idx).zfill(4) + ".bin")
