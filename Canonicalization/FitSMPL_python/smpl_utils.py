...
def iterative_optimize_on_vertices(model,
    ...
    for i in range(itr):
        ...
        model_nmle = ComputeNormales(model.r, model.f)
        vertices_corr = np.zeros((model.r.shape[0], 3))
        weights_corr = np.zeros((model.r.shape[0]))
        count = 0
        for v in model.r:
            '''vertices_corr[count, :] = v
            weights_corr[count] = 0.1
            count = count + 1'''
            ...
            # validity test on normals
            nmle = model_nmle[count, :]
            nmle_corr = normales[ii,:]
            if (np.dot(nmle,nmle_corr) < 0.0):
                weights_corr[count] = 0.0
            count = count + 1
        ...
        if opt_cross:
            ...
            for j in range(vertices.shape[0]):
                ...
                # validity test on normals
                nmle = model_nmle[ii, :]
                nmle_corr = normales[j,:]
                if (np.dot(nmle,nmle_corr) < 0.0):
                    weights_cross_corr[count] = 0.0
                count = count + 1
            ...
            if i > 3:
                print("ignoring input 3D joints")
                optimize_on_vertices(
                    model=model,
                    vertices=vertices_corr,
                    weights_corr=weights_corr,
        ...
        if opt_target == "trans":
            chumpy.minimize(
                ...
                options={'maxiter': 1000,
                         'e_3': .0001,
                         'disp': 0})
        elif opt_target == "pose_and_betas":
            chumpy.minimize(
                objs,
                x0=[model.pose , model.betas],
                method='dogleg',
